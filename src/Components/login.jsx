...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    await axios.post("http://localhost:4000/user/login", payload).then((data) => {
    ...
    <div>
      ...
      <div className="w-full max-w-sm my-28 p-6 m-auto mx-auto bg-white rounded-lg shadow-md dark:bg-gray-800">
        ...
        <form className="mt-6">
          ...
          <p className="px-6 pt-4 text-sm text-center dark:text-gray-600 font-semibold text-gray-600 ">Already have an account ?
            <a rel="noopener noreferrer" href="/registartion" className="hover:underline  text-gray-800 hover:text-[var(--purple-color)] ">Sign up</a>.
          </p>
        </form>
